...
// 플레이어에 해당하는 소환사가 소환한 몬스터이다. 적 몬스터와 적 소환사를 공격한다.
public class PlayerMonster extends Monster{
	...
	public PlayerMonster(Image image, int health, Point location, int coin, int range, int speed,
	...
	// 임시로 쓰는 생성자.
	public PlayerMonster(Image image,Point location) {
		super(image,10,location,0,50,5,3,40,30);
//		각 파라미터는 image, health, location, coin, range, faction , speed, attackPoint, cost,delay;
	}
	// 이동 메소드
	public void move(Summoner summoner) { // 몬스터 이동을 위한 메소드
		int totalSpeed = speed;
		if(summoner.inventory[1] > 0) {
			totalSpeed += SpeedUp.applyEffect();						
		}
		coordinate.x += totalSpeed; // x 좌표에 10만큼 몬스터 이동
	}
	...
	public void findEnemy(ArrayList<EnemyMonster> enemyHorde, EnemySummoner enemyBase){
		...
		// 적의 가로 길이, 아군의 가로길이. 사거리를 잴때 필요하다.
		int enemyHorizontalSize = FrameMain.ImageWidthValue("images/blue_snail.png");
		...
		if(enemyBase.healthPoint > 0 && (coordinate.x+allyHorizontalSize + range >= 800 - 80)) {
			...
			for(int i=0; i<enemyHorde.size(); i++){
				...
				if(coordinate.x + allyHorizontalSize +range >= enemyHorde.get(i).coordinate.x ) {
					isEnemyDetected = true;
	...
	public int attackEnemy(ArrayList<EnemyMonster> enemyHorde,EnemySummoner enemyBase) {
		int coin = 0;
		...
		if(target == -1) {
			...
			soundEffect = new Music("punch_effect.mp3",false);
			soundEffect.start();
			// 기지를 공격한다면
